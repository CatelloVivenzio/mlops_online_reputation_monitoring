...
# Fixture MLOPS per gestire lifespan
@pytest.fixture(scope="module", autouse=True)
def client():

    """
    Inizializza il TestClient attivando l'evento lifespan di FastAPI.
    Questo assicura che il modello SentimentModel 
    sia caricato in memoria prima dei test.
...
def test_analyze_sentiment_positive(client):
    ...
    assert response.status_code == 200
    # Verifica che il modello riconosca il sentiment positivo nel testo
    assert "positive" in response.text.lower()

@patch("src.api.generate_drift_report")
@patch("os.path.exists")
def test_report_endpoint_success(mock_exists, mock_generate, client):
...
@patch("src.api.generate_drift_report")
@patch("os.path.exists")
def test_report_endpoint_file_missing(mock_exists, mock_generate, client):
